...
export default function DayIntervalCalendar({
  calendarData,
  selectedSlots,
  onSlotSelect,
  onSlotDeselect
}: DayIntervalCalendarProps) {
  const [currentWeekStart, setCurrentWeekStart] = useState(new Date(calendarData.weekStart))
  const [selectedDay, setSelectedDay] = useState<string | null>(null)
  const [timeInterval, setTimeInterval] = useState({ from: '08:00', to: '17:00' })
  ...
  const handleDayClick = (date: Date) => {
    ...
    // Check if day has available intervals
    const daySlots = calendarData.slots.filter(s => s.date === dateStr && s.fromTime && s.toTime)
    if (daySlots.length > 0) {
      setSelectedDay(dateStr)
    } else {
      // If no specific intervals, allow selecting the day with custom interval
      setSelectedDay(dateStr)
    }
  }
  ...
  const handleConfirmInterval = () => {
    ...
    const slot: KalenderSlot = {
      ...
      toTime: timeInterval.to,
      status: getSlotStatus(new Date(selectedDay))
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <Dialog open={selectedDay !== null} onOpenChange={(open) => !open && setSelectedDay(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Velg tidsintervall</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                onChange={(e) => setTimeInterval({ ...timeInterval, from: e.target.value })}
                className="mt-1"
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) => setTimeInterval({ ...timeInterval, to: e.target.value })}
                className="mt-1"
              />
            </div>
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleConfirmInterval}>
                Bekreft
              </Button>
            </div>
          </div>
